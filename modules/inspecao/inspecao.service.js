...
function listEquipamentosAtivos() {
  const fromDb = db.prepare("SELECT id, nome FROM equipamentos WHERE ativo = 1 ORDER BY nome").all();
  const merged = new Map();

  for (const nome of DEFAULT_EQUIPAMENTOS) {
    const chave = normalizeText(nome);
    merged.set(chave, { id: null, nome, chave });
  }
  ...
  for (const item of fromDb) {
    ...
    if (!nome) continue;
    const chave = normalizeText(nome);
    merged.set(chave, { id: item.id || null, nome, chave });
  }
...
function syncFromOS(osId, user = null) {
  ...
  const dataRef = os?.data_inicio || os?.data_fim;
  if (!dataRef) return { synced: false, reason: "os_or_data_missing" };
  const period = monthYearFromDate(dataRef);
  ...
  const equipamento = resolveEquipamentoFromOS(os, buildEquipmentMap(equipamentos))
    || { id: os.equipamento_id || null, nome: String(os.equipamento_nome || "Equipamento não mapeado"), chave: normalizeText(os.equipamento_nome || "equipamento-nao-mapeado") };

  const dia = dayFromDate(dataRef);
  ...
  if (status === "NC") {
    upsertNaoConformidade(inspecao.id, equipamento, String(dataRef).slice(0, 10), os, {
...
function buildMatrix(inspecaoId, ano, mes, equipamentos) {
  ...
  itens.forEach((item) => {
    if (!byEquip.has(item.equipamento_nome)) {
      byEquip.set(
