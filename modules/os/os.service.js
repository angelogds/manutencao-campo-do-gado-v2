  ...
  try {
    const existing = db
      .prepare(
        `SELECT id
         FROM os_execucoes
         WHERE os_id=? AND finalizado_em IS NULL
         ORDER BY id DESC
         LIMIT 1`
      )
      .get(Number(osId));
    ...
    if (!existing) {
      db.prepare(
        `INSERT INTO os_execucoes (os_id, mecanico_user_id, iniciado_em)
         VALUES (?, ?, datetime('now'))`
      ).run(Number(osId), Number(userId));
    }
  } catch (_e) {
    // se não existir tabela os_execucoes, ignoramos
  }
}
...
function listGrauOptions() {
  ...
  try {
    const rows = db
      .prepare(
        `SELECT DISTINCT UPPER(COALESCE(${grauColumn},'')) AS v
         FROM os
         WHERE ${grauColumn} IS NOT NULL AND TRIM(${grauColumn}) <> ''
         LIMIT 20`
      )
      .all();
...
// ✅ Cria OS (corrigido, sem duplicações)
function createOS({ equipamento_id, equipamento_texto, descricao, tipo, opened_by, grau }) {
  ...
  const t = String(tipo || "CORRETIVA").trim().toUpperCase();
  ...
  webPushService
    .sendOSPushNotifications({
      osId,
      equipamento: os.equipamento || equipamentoFinal,
      grau:
        os.grau ||
        os.grau_dificuldade ||
        os.nivel_grau ||
