...
function lista(req, res) {
  const status = [STATUS.ABERTA, STATUS.EM_COTACAO, STATUS.COMPRADA].includes(req.query.status) ? req.query.status : STATUS.ABERTA;
...
function pdf(req, res) {
  try {
    ...
    console.error("❌ PDF compras:", e && (e.stack || e.message || e));
    req.flash("error", "Não foi possível gerar o PDF agora. Verifique o servidor (pdfkit).");
