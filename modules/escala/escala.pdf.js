...
function generatePeriodPDF({ start, end, baseServicos = [], apuracao = [], registros = [], descricoes = [] } = {}) {
  const doc = createDoc();
  const meta = {
  ...
  process.nextTick(() => {
    ...
    doc.fillColor(COLORS.text).font("Helvetica-Bold").fontSize(10)
      .text(`Período: ${formatDateBr(start)} até ${formatDateBr(end)}`, PAGE.margins.left, doc.y);
    doc.y += 14;
    ...
    if (!baseServicos.length) {
      ...
      doc.font("Helvetica").fontSize(9).fillColor(COLORS.text)
        .text("Sem registros de serviço no período.", PAGE.margins.left, doc.y);
      doc.y += 14;
    } else {
    ...
    drawTable(doc, {
      meta,
      columns: [
        { key: "colaborador", label: "Colaborador", width: 165 },
        { key: "horas", label: "Total horas", width: 88, align: "center" },
        { key: "inteiras", label: "Total dias", width: 88, align: "center" },
        { key: "meias", label: "Meias folgas", width: 92, align: "center" },
        { key: "saldo", label: "Saldo", width: 116, align: "center" },
      ],
      rows: apuracao.map((a) => ({
      ...
      emptyRow: { colaborador: "Sem apuração no período.", horas: "-", inteiras: "-", meias: "-", saldo: "-" },
    });

    doc.y += 6;
    ensureSpace(doc, 20, meta);
    doc.font("Helvetica-Bold").fontSize(11).fillColor(COLORS.greenDark)
      .text("3. Concessão das folgas", PAGE.margins.left, doc.y);
    doc.y += 14;

    drawTable(doc, {
      meta,
      columns: [
        { key: "data", label: "Data", width: 74, align: "center" },
        { key: "tipo", label: "Tipo", width: 94, align: "center" },
        { key: "concessao", label: "Concessão", width: 88, align: "center" },
        { key: "motivo", label: "Motivo", width: 154 },
        { key: "colaborador", label: "Colaborador", width: 139 },
      ],
      ...
      emptyRow: { data: "-", tipo: "-", concessao: "-", motivo: "Sem registros de folga/atestado/férias no período.", colaborador: "-" },
    });
